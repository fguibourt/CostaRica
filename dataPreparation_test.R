...
setwd("C:/Users/Vincent/Documents")

## load data
data_test = as_data_frame(read.csv("test.csv", sep = ",", header = T))
...
data_test[factors] = lapply(data_test[factors], as.factor)
...
data_test[factors] = lapply(data_test[factors], as.character)
data_test[factors] = lapply(data_test[factors], as.numeric)
...
data_group_test = data_temp_test %>%
  group_by(idhogar) %>%
  summarise(
    nb_parentfamily = 
      sum(parent_family),
...
data_temp_test = data_temp_test %>%
  left_join(data_group_test, by = "idhogar")
...
col_ft = c("Id", "idhogar", "overcrowding", "edjefi", "hacdor", "rooms","hacapo","v14a","refrig","r4h3","r4m3","hhsize","dependency",
...
data_ft_test[col_ft_factors] = lapply(data_ft_test[col_ft_factors], as.factor)

## if you want to remove some columns : fill the -c() argument like -c(idhogar, Id, hacapo)
data_ft_test = subset(data_ft_test, select = -c(idhogar, Id))

data_ft_test = na.omit(data_ft_test)
...
data_ft_test = select(data_ft_test, -col_delete)
...
data_ft_test = select(data_ft_test, col_selected)